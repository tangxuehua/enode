...
namespace BankTransferSample.EventHandlers
{
        ...
        IMessageHandler<TransferTransactionCanceledEvent>
    {
        ...
        {
            _logger.InfoFormat("账户已创建，账户：{0}，所有者：{1}", evnt.AggregateRootId, evnt.Owner);
        ...
        {
            _logger.InfoFormat("转账交易已开始，交易ID：{0}，源账户：{1}，目标账户：{2}，转账金额：{3}", evnt.AggregateRootId, evnt.TransactionInfo.SourceAccountId, evnt.TransactionInfo.TargetAccountId, evnt.TransactionInfo.Amount);
            return Task.CompletedTask;
        }
        ...
        {
            _logger.InfoFormat("转账交易已完成，交易ID：{0}", evnt.AggregateRootId);
            return Task.CompletedTask;
        }
